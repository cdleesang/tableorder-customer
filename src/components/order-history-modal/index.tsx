...
function OrderHistoryModal() {
  ...
  useEffect(() => {
    api.functional.order.getOrderHistoriesByTableId(connection, parseInt(connection.headers?.tid || '0', 10))
      .then(({ orderHistories: histories }) => {
        const parsedHistories = histories.reduce((prev, next) => {
          if (prev.length === 0) {
            return [{
    ...
    <Modal className="order-history" close={() => setIsOrderHistoryModalOpen(false)}>
      <div className="order-history-header">
        <h2 className="order-history-title">
          주문 내역
          <div className="desc" style={{ fontSize: '.6em', opacity: 0.5 }}>
        ...
        </h2>
        {
          ...
          && <span className="order-total-price">
            총 주문 금액: {
              priceComma(
                orderHistories.reduce(
                  (acc, orderHistory) => acc + orderHistory.totalPrice,
                  0,
                ),
              )}원
          </span>
